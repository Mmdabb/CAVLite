...
public:
	...
	template <class T> bool GetValueByFieldName(string field_name, T& value, bool NonnegativeFlag = false)  //true
	{
		if (FieldsIndices.find(field_name) == FieldsIndices.end())
		{
			return false;
...
public:
	...
	void WriteTextString(const std::string& textString)
	{
		if (!outFile.is_open())
			return;
	...
	void WriteTextLabel(const std::string& textString)
	...
	template <class T> void WriteParameterValue(const std::string& textString, T value)
	{
		if (!outFile.is_open())
			return;
