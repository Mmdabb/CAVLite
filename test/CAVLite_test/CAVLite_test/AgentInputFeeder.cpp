...
#include "CSVParser.h" 
#define _CRT_SECURE_NO_WARNINGS  // Optional: suppress deprecation warning
#include <cstdlib>
...
void AgentInputFeeder::loadAllAgentsFromCSV() {
    ...
    while (parser.ReadRecord()) {
        ...
        input.node_sequence = _strdup(node_sequence.c_str());
...
AgentInputFeeder::~AgentInputFeeder() {
    for (auto& a : all_agents) {
        if (a.node_sequence) {
            free((void*)a.node_sequence);  // cast away const-ness
        }
    }
}
